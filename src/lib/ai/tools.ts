...
export const createReadOnlyTools = (baseUrl: string, headers?: HeadersInit) => {
  ...
  const leaderboardTool = tool(
    ...
    },
    {
      ...
      description: "获取排行榜数据（进球/助攻/红黄牌/点球/乌龙/扑救/出场）。",
      schema: z.object({
        type: z.enum(["goals", "assists", "yellow_cards", "red_cards", "penalty_goals", "penalty_misses", "own_goals", "saves", "appearances"]).optional(),
