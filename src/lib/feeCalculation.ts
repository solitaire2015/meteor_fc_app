 ...
 * - Video fee follows Excel formula: ROUNDUP(totalTime/3*2, 0)
 * - Late fee is 10 yuan if any late arrival
 */
...
export function calculatePlayerFees(input: FeeCalculationInput): FeeCalculationResult {
  const { attendanceData, isLateArrival, feeCoefficient } = input;
  ...
  const lateFee = isLateArrival ? 10 : 0;
  // Video fee follows Excel formula: ROUNDUP(totalTime/3*2, 0)
  const videoFee = Math.ceil(normalPlayerParts / 3 * 2);
