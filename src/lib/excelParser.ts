...
export function parseExcelData(rawData: any[][], sheetName: string): ExcelMatchData {
  ...
  for (let i = dataStartRow; i < (totalsRowIndex > 0 ? totalsRowIndex : rawData.length); i++) {
    ...
    try {
      ...
      const totalFeeCol = hasShortIdColumn ? 19 : 18
      ...
      const playerData: ExcelPlayerData = {
        序号: parseFloat(row[0]) || 0,
        姓名: playerName,
        shortId: shortId,
        section1,
        section2,
        section3,
        totalTime: parseFloat(row[totalTimeCol]) || 0,
        fieldFee: parseFloat(row[fieldFeeCol]) || 0,
        onTime,
        lateFee: parseFloat(row[lateFeeCol]) || 0,
        videoFee: parseFloat(row[videoFeeCol]) || 0,
        totalFeeCalculated: parseFloat(row[totalFeeCol]) || 0,
  ...
  if (headerRow) {
    for (let col = headerRow.length - 1; col >= 0; col--) {
      if (headerRow[col] && typeof headerRow[col] === 'string') {
        const scoreMatch = headerRow[col].match(/(\d+):(\d+)/)
        if (scoreMatch) {
  ...
  if (totalsRowIndex >= 0) {
    ...
    if ((typeof secondToLastCol === 'number' || !isNaN(parseFloat(secondToLastCol))) && 
        (typeof lastCol === 'number' || !isNaN(parseFloat(lastCol)))) {
      // New format with separate columns
      fieldFeeTotal = typeof secondToLastCol === 'number' ? secondToLastCol : parseFloat(secondToLastCol)
      waterFeeTotal = typeof lastCol === 'number' ? lastCol : parseFloat(lastCol)
    } else {
