...
export async function POST(
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const participations = Array.from(playerMap.values())
        ...
        .map(player => {
          ...
          const fees = calculatePlayerFees({
            ...
            feeCoefficient: match.feeCoefficient
          })
      ...
      // 3. Save match events (goals and assists)
      const events = []
...
export async function GET(
  ...
  try {
    ...
    // Also collect participation notes for the FinancialCalculator
        ...
        for (let part = 1; part <= 3; part++) {
          ...
          if (value > 0) {
            attendanceData.push({
              ...
              notes: p.notes || undefined // Include notes in attendance data as well
            })
          }
        }
      }
